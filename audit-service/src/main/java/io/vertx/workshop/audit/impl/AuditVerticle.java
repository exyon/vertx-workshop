...
public class AuditVerticle extends RxMicroServiceVerticle {
  ...
  public void start(Future<Void> future) {
    ...
    Single<MessageConsumer<JsonObject>> readySingle = Single.error(new Exception(""));
  ...
  private Single<Void> initializeDatabase(boolean drop) {
    ...
    // To handle such a process, create an RxJava Single and compose it with the RxJava flatMap operation:
    // retrieve the connection -> drop table -> create table -> close the connection
    // For this  use `Func1<X, Single<R>>`that takes a parameter `X` and return a `Single<R>` object.
